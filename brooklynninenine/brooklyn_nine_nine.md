...
I was unsure what tool to use to analyze and extract hidden data from an image so I searched up and found a tool called stegcracker which is a bruteforce tool for steg files . 
...
I initally forgot to perform a network scan, however I was able to uncover open ports. The three open ports are FTP, SSH, and HTTP which is the website. In order to open the SSH we need a username.  Let's look to see if FTP is public.

![alt text](image-3.png)

Looking into FTP, we are able to access the files anonymously.
...
When I enter the password into jake's ssh it ends up being wrong. So I tryed it for the other two user which also didn't work. Maybe I could try bruteforcing the SSH since amy said that jake's password is weak. For this bruteforce I will be using hydra which is a password cracker tool. I will use jake as the user and rockyou as the password list. 
...
the first thing i did was searched up any vulnerabilties with this command line. 

![alt text](image-10.png)

one of the first website that popped up was GTFOBins which has a list of different executables that can bypass security restrictions from misconfigured systems. 
